...
namespace ClinicApp.Interfaces.Payment.POS
{
    ...
    public interface IPosManagementService
    {
        ...
        /// <returns>نتیجه اعتبارسنجی</returns>
        Task<ServiceResult<bool>> ValidatePaymentAsync(int receptionId, decimal amount);

        /// <summary>
        /// ثبت پرداخت POS
        /// </summary>
        /// <param name="receptionId">شناسه پذیرش</param>
        /// <param name="posPayment">اطلاعات پرداخت POS</param>
        /// <returns>نتیجه ثبت پرداخت</returns>
        Task<ServiceResult<bool>> RegisterPosPaymentAsync(int receptionId, PosPaymentDto posPayment);

        /// <summary>
        /// ثبت پرداخت نقدی
        /// </summary>
        /// <param name="receptionId">شناسه پذیرش</param>
        /// <param name="cashPayment">اطلاعات پرداخت نقدی</param>
        /// <param name="sessionId">شناسه جلسه نقدی</param>
        /// <returns>نتیجه ثبت پرداخت</returns>
        Task<ServiceResult<bool>> RegisterCashPaymentAsync(int receptionId, CashPaymentDto cashPayment, int sessionId);

        /// <summary>
        /// دریافت جلسه نقدی باز
        /// </summary>
        /// <param name="userId">شناسه کاربر</param>
        /// <returns>جلسه نقدی باز</returns>
        Task<ServiceResult<CashSession>> GetOpenCashSessionAsync(string userId);
