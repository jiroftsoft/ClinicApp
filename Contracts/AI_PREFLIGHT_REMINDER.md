# 📌 متن یادآوری برای هر Chat جدید

**این متن را در ابتدای هر مکالمه جدید Paste کن:**

---

```
🤖 AI: قبل از هر پاسخ، این قرارداد الزامی است:

📄 Contracts/AI_EXECUTION_CONTRACT.md را بخوان (10s)
🚨 Contracts/CRITICAL_MODULE_SAFETY_CONTRACT.md را بخوان (20s) ← ⭐ جدید
⚡ Contracts/AI_PREFLIGHT_QUICK_V3.md را چک کن (30s)

🚫 15 ممنوعیت + NO BLIND CHANGES
💰 مالی؟ → Contracts/AI_PREFLIGHT_MASTER_V3.md → STEP 2
🐛 باگ؟ → Contracts/AI_PREFLIGHT_MASTER_V3.md → STEP 3
🔴 تغییر حیاتی؟ → STOP & ASK USER FIRST
🚨 تعارض؟ → HARD STOP

✓ Factory Pattern + ServiceResult + Log + Test + Doc
✓ هرگز تغییر حیاتی بدون درک منطق + تأیید کاربر
```

---

## 📋 **نسخه کامل (اختیاری):**

```
🤖 AI Preflight Protocol - الزامی

قبل از هر پاسخ:
1. Contracts/AI_EXECUTION_CONTRACT.md (10s)
2. Contracts/CRITICAL_MODULE_SAFETY_CONTRACT.md (20s) ← ⭐ جدید: NO BLIND CHANGES
3. Contracts/AI_PREFLIGHT_QUICK_V3.md (30s)
4. Knowledge-Base (اگر نیاز به Helper/Standard):
   - Contracts/Knowledge-Base/AI/Master/README.md
   - Contracts/Knowledge-Base/AI/Master/INDEX.md
   - Contracts/Knowledge-Base/AI/Master/01-Helpers-DateTime.md (اگر تاریخ)
   - Contracts/Knowledge-Base/AI/Master/03-Development-Contract-Quick-Guide.md
   - Contracts/Knowledge-Base/AI/Master/05-Debugging-Specialist-Contract.md (اگر باگ)
5. چک 15 ممنوعیت AI No-Fly Zone
6. شناسایی نوع کار:
   - معمولی → Quick Preflight
   - مالی → STEP 2 (10 قانون طلایی)
   - باگ → STEP 3 (6 مرحله)
   - تغییر حیاتی → STOP & ASK USER FIRST
7. رعایت الزامات:
   ✓ Factory + ServiceResult + try-catch + Log + Validation
   ✓ [Authorize] + [ValidateAntiForgeryToken]
   ✓ Standards (رنگ/فونت/تاریخ/پیام)
   ✓ استفاده از Helpers از Knowledge-Base
8. HARD STOP در صورت تعارض

مراجع:
- Contracts/AI_PREFLIGHT_QUICK_V3.md
- Contracts/AI_PREFLIGHT_MASTER_V3.md
- Contracts/AI_PREFLIGHT_INDEX.md
- Contracts/Knowledge-Base/AI/Master/ (Knowledge-Base)
```

---

## ⚡ **نسخه خلاصه (توصیه می‌شود):**

```
🤖 AI: رعایت Contracts الزامی است:
- AI_EXECUTION_CONTRACT.md
- CRITICAL_MODULE_SAFETY_CONTRACT.md ← ⭐ جدید: NO BLIND CHANGES
- AI_PREFLIGHT_QUICK_V3.md (30s)
- Knowledge-Base (اگر نیاز به Helper/Standard):
  * Contracts/Knowledge-Base/AI/Master/README.md
  * Contracts/Knowledge-Base/AI/Master/INDEX.md

مالی = STEP 2 | باگ = STEP 3 | تغییر حیاتی = ASK FIRST | تعارض = HARD STOP
```

---

## 🎯 **استفاده:**

1. **در ابتدای هر Chat جدید** این متن را Paste کن
2. AI خودکار تمام قراردادها را چک می‌کند
3. فایل‌ها در `Contracts/` هستند

---

## 📚 **سلسله‌مراتب اولویت:**

```
1. CRITICAL_MODULE_SAFETY_CONTRACT.md ← ⭐ بالاترین اولویت
2. CRITICAL-FINANCIAL-MODULE-CONTRACT.md (اگر مالی)
3. AI_EXECUTION_CONTRACT.md
4. AI_PREFLIGHT_MASTER_V3.md (اگر پیچیده)
5. AI_PREFLIGHT_QUICK_V3.md (اگر معمولی)
6. Knowledge-Base (اگر نیاز به Helper/Standard)
```

**در صورت تعارض:**
- CRITICAL_MODULE_SAFETY_CONTRACT.md اولویت دارد
- سپس CRITICAL-FINANCIAL-MODULE-CONTRACT.md (اگر مالی)
- سپس AI_EXECUTION_CONTRACT.md

---

## 💡 **نکات مهم:**

- ✅ نسخه خلاصه را در chat جدید Paste کن
- ✅ AI خودکار تمام قراردادها را چک می‌کند
- ✅ فایل‌ها در `Contracts/` هستند
- ✅ همیشه قبل از تغییرات حیاتی: STOP & ASK USER FIRST
- ✅ NO BLIND CHANGES - هیچ تغییری بدون درک کامل منطق

---

**🎯 با این متن، AI همیشه قراردادها را رعایت می‌کند!**

